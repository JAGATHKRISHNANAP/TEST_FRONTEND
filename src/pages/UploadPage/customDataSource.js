...
// import DraggableTable from '../../components/customJoin/DraggableTable';
// import DropZone from '../../components/customJoin/dropzone';
// import JoinConfigSection from '../../components/customJoin/JoinConfigSection';
...
//   //         try {
//   //           const tableConnectionType = getTableConnectionType(table);
//   //           const columns = await fetchColumnsAPI(table, databaseName, tableConnectionType,selectedUser);
//   //           fetchedColumns[table] = columns;
//   //         } catch (error) {
//   //           console.error(`Error in fetchColumns for table ${table}:`, error);
//   //         }
...
//   //   fetchColumns();
//   // }, [tableNames, databaseName, tableConnectionType,droppedTables, selectedUser]); 
//   // useEffect(() => {
//   //   const fetchColumns = async () => {
//   //     const fetchedColumns = {};
...
//   //         try {
//   //           const tableConnectionType = getTableConnectionType(table);
//   //           // Ensure selectedUser is valid for external DB
//   //           if (tableConnectionType === "external" && !selectedUser) {
//   //             throw new Error(
//   //               `Connection details are required for table ${table} with external database.`
//   //             );
//   //           }
...
//     tableNames.map(async (table) => {
//       try {
//         const tableConnectionType = getTableConnectionType(table); // Get connection type for the current table

//         if (tableConnectionType === "external" && !selectedUser) {
//           throw new Error(`Connection details are required for table ${table} with external database.`);
//         }

//         // Fetch columns based on connection type
//         const columns = await fetchColumnsAPI(
//           table,
//           databaseName,
//           tableConnectionType, // Use the connection type for column fetch
//           selectedUser
//         );

//         // Merge the new columns with the existing columns in state
//         fetchedColumns[table] = columns;
//       } catch (error) {
//         console.error(`Error in fetchColumns for table ${table}:`, error);
//       }
...
//   // Merge newly fetched columns with the existing state (preserve previous state)
//   setColumns((prevColumns) => ({
//     ...prevColumns,
//     ...fetchedColumns, // Merge fetched columns into the existing state
//   }));
// };

// // Use effect to call the fetchColumns function whenever the table names or selected user change
// useEffect(() => {
//   fetchColumns();
// }, [tableNames, databaseName, droppedTables, selectedUser]); 
//   const getTableConnectionType = (table) => {
...
//     }
//     if (isChecked) {
//       // Add the column if checked and not already present
//       if (!updated[tableName].includes(columnName)) {
//         updated[tableName].push(columnName);
//       }
//     } else {
...
//         (col) => col !== columnName
//       );
//       // If no columns are selected for a table, delete the entry
...
import { DndProvider, useDrag, useDrop } from 'react-dnd';
import { HTML5Backend } from 'react-dnd-html5-backend';
import { Box, Typography, InputLabel, AppBar,Toolbar,MenuItem, Button,Menu,FormControl, IconButton, NativeSelect ,TextField , Checkbox,Grid, FormControlLabel,Dialog, DialogTitle, DialogContent, DialogActions,Card, CardContent, CardHeader} from '@mui/material';
import DeleteIcon from '@mui/icons-material/Delete';
import './custom.css';
import ArrowDropDownIcon from '@mui/icons-material/ArrowDropDown';
import Select from '@mui/material/Select';
...
const CustomJoinWithFetchTables = () => {
    ...
    const fetchTableNames = async () => {
      try {
        ...
        if (selectedUser === 'Local db'  || selectedUser === null) {
          ...
          const data = await fetchTableNamesAPI(databaseName); // Fetch from the local database
          fetchedData = data.map((table) => ({
            tableName: table,          // Store the table name
            connectionType: 'Local db' // Connection type is 'Local db'
          }));
          setTableNames(data);
          console.log("fetchedData",fetchedData)
        } else {
          ...
          try {
            ...
            fetchedData = data.map((table) => ({
              tableName: table,           // Store the table name
              connectionType: 'external'  // Connection type is 'external'
            }));
            setTableNames(data.sort()); // Sorting the fetched tables if needed
            console.log("fetchedData",fetchedData)
          } catch (error) {
  ...
  const constructSQLQuery = () => {
    const tables = droppedTables.map((table) => table.name); 
    const tableConnectionType = getTableConnectionType(droppedTables.map((table) => table.name)); // Get connection type for the current table
  console.log("tableConnectionType",tableConnectionType)
    const formattedSelectedColumns = Object.entries(selectedColumns).flatMap(
      ([tableName, columns]) =>
        columns.map((column) => `${tableName}.${column}`) 
    );
