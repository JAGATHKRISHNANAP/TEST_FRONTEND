...
const AreaChart = ({ categories = [], values = [], aggregation = "Aggregation", x_axis="X_axis", y_axis="Y_axis", otherChartCategories = [] }) => {
    ...
    // Function to handle data point selection
    const handleClicked = (event, chartContext, config) => {
        if (config && config.dataPointIndex !== -1) {
            const clickedIndex = config.dataPointIndex;
            const clickedCategory = categories[clickedIndex];
            console.log("Clicked category:", clickedCategory); // Log the clicked category
            dispatch(updateSelectedCategory(clickedCategory));
            sendClickedCategory(clickedCategory); // Send the clicked category to the backend (if needed)
            setShowResetButton(true);
        } else {
            console.warn("Invalid click event or data point index.");
        }
    };
    ...
    const options = {
        chart: {
            events: {
                dataPointSelection: handleClicked, // Attach the event handler here
            },
            toolbar: {
                show: false,
            },
