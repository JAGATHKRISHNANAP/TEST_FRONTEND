...
import '../Style.css';
import {
    setXAxis, setYAxis, setAggregate, setFilterOptions, setCheckedOptions, setShowFilterDropdown, setSelectAllChecked, generateChart
} from '../../features/Dashboard-Slice/chartSlice';
...
function DuealChartInput() {
    const dispatch = useDispatch();
    const { xAxis, yAxis, aggregate, checkedOptions} = useSelector(state => state.chart);
    const chartType = useSelector(state => state.chartType.type);
    const SelectedTable = useSelector((state) => state.dashboard.checkedPaths);
    const barColor = useSelector((state) => state.chartColor.chartColor);
    const databaseName = localStorage.getItem('company_name');
    ...
    return (
        <div className="App">
            <div className="dash-right-side-container">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', zIndex: 1000 }}> {/* Columns and Aggregation in one row */}
                    <XAxisInput
                        xAxis={xAxis}
                ...
                <div style={{ display: 'flex', alignItems: 'center', zIndex: 1000 }}> {/* Y-axis and Audio in one row */}
                    <YAxisInput
                        yAxis={yAxis}
                        MAX_ROW={MAX_ROW}
                        errorMessage={errorMessage}
