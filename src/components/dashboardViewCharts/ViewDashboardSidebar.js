...
//           const updatedChartData = response.chart_datas.map(chart => {
//             const chartId = chart.chart_id ? chart.chart_id.toString() : null;
//             const position = chart.positions || { x: 0, y: 0 };
...
function ViewDashboardSidebar() {
  ...
  // Modal and Snackbar states for chart deletion
  const [openModal, setOpenModal] = useState(false);
  const [chartToDelete, setChartToDelete] = useState(null);
  const [anchorEl, setAnchorEl] = useState(null);
  ...
  // Fetch total rows (chart names) when component mounts or user_id changes
  useEffect(() => {
    const company_name = localStorage.getItem("company_name");
    console.log("company_name:", company_name);
    ...
    dispatch(fetchDashboardTotalRows(user_id))
      ...
      .then((response) => {
        ...
        if (response && response.chart_names && typeof response.chart_names === "object") {
          ...
          console.error(
            "chart_names is not an object or has unexpected structure:",
  ...
  // Handle chart button click (for both dashboard and sidebar)
  const handleChartButtonClick = (chartNumber, chartName) => {
    setActiveChart(chartName);
    dispatch(clearDashboardCharts());
    ...
    dispatch(fetchDashboardData(chartName))
      ...
      .then((response) => {
        if (response && response.chart_datas) {
          // Process each chart: convert chart_id to string and set position
          const updatedChartData = response.chart_datas.map((chart) => {
            const chartId = chart.chart_id ? chart.chart_id.toString() : null;
            const position = chart.position || { x: 0, y: 0 };
            return { ...chart, chart_id: chartId, position: position };
          });
          console.log("chart psoition",updatedChartData)

          // Dispatch each chart according to its type
          updatedChartData.forEach((chartDataItem) => {
            if (chartDataItem.chart_type === "singleValueChart") {
              dispatch(addTextChart(chartDataItem));
            } else {
              dispatch(addChartData(chartDataItem));
            }
          });
        }
      })
  ...
  // Context menu handling for deletion
  const handleContextMenu = (event, chartName, index) => {
    event.preventDefault();
    setAnchorEl({
      mouseX: event.clientX + 2,
  ...
  return (
    <div className="App" style={{ height: "100vh" }}>
      {/* Grid container for chart buttons */}
      <Box
        sx={{
          display: "grid",
          ...
          width: "100%",
          height: "100%",
          position: "relative", // So absolute positioned buttons are relative to this container
        }}
      >
        {chartData &&
          chartData.map((chart, index) => {
            // Retrieve the chart's position. If not set, default to { x: 0, y: 0 }.
            const pos = chartPositions[chart.chart_id] || { x: 0, y: 0 };
            return (
              <Button
                key={chart.chart_id || index}
                ...
                onClick={() => handleChartButtonClick(index + 1, chart.chart_heading)}
                onContextMenu={(event) =>
                  handleContextMenu(event, chart.chart_heading, index)
                }
                disabled={activeChart === chart.chart_heading}
              >
                {chart.chart_heading}
